...
class KMeans private (
   ...
   */
  @Since("0.8.0")
  def this() = this(100, 1000000, 1, KMeans.K_MEANS_PARALLEL, 5, 1e-4, Utils.random.nextLong())
  ...
  private def runAlgorithm(data: RDD[VectorWithNorm]): KMeansModel = {
    ...
    logInfo(s"Initialization with $initializationMode took " + "%.3f".format(initTimeInSeconds) +
      " seconds.")
    println(s"Initialization with $initializationMode took " + "%.3f".format(initTimeInSeconds) + " seconds." )  
      ...
      for ((run, i) <- activeRuns.zipWithIndex) {
        ...
        if (!changed) {
          ...
          logInfo("Run " + run + " finished in " + (iteration + 1) + " iterations")
       	  println("Run " + run + " finished in " + (iteration + 1) + " iterations")
        }
    ...
    println(s"Iterations took " + "%.3f".format(iterationTimeInSeconds) + " seconds.")
    val totalTime = iterationTimeInSeconds  + initTimeInSeconds 
    println(s"Total time taken " + "%.3f".format(totalTime) + " seconds.")
    println("maximum iterations are : " + maxIterations)
    ...
    println(s"The cost for the best run is $minCost.")
    ...
    centers.foreach { center =>
      ...
      if (lowerBoundOfSqDist < bestDistance) {
        ...
        if (distance < bestDistance) {
          ...
          bestIndex = i
	  println(s"bestDistance is " + "%.3f".format(bestDistance) + "for bestIndex " + bestIndex)
        }
      }
      i += 1
    }
